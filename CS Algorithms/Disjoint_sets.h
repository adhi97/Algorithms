...
int Disjoint_set::find_set(int item) const{
    ...
    return nodes[item]->ptr_to_info->head->content; //yes?
}
...
void Disjoint_set::union_sets(int node_index1, int node_index2) {
    ...
    nodes[ni1]->ptr_to_info->size = nodes[ni2]->ptr_to_info->size + nodes[ni1]->ptr_to_info->size;
    
    ll_entry* temp = nodes[ni2]->ptr_to_info->head;
    
    //ll_entry* temp = nodes[ni2]; //which way do i do this? this or the one above?
    
    nodes[ni1]->ptr_to_info->tail->next = nodes[ni2]->ptr_to_info->head;
    
    nodes[ni1]->ptr_to_info->tail = nodes[ni2]->ptr_to_info->tail;
    
    nodes[ni1]->ptr_to_info->tail->next = nullptr;
    
    set_info* toBeDeleted = nodes[ni2]->ptr_to_info;
    
	/* iterate through the shorter list and modify the pointers */
	while (temp != nullptr) {
        temp->ptr_to_info = nodes[ni1]->ptr_to_info;
        temp = temp->next;
	}

    sets[nodes[ni2]->content] = nullptr;
    ...
    //nodes[ni2]->ptr_to_info = nodes[ni1]->ptr_to_info;
}
